...
#define LOCAL_MIYABI_IPTABLE_PERMIT_PATH_2 "/system/bin/netmgrd"
#ifdef CONFIG_SECURITY_MIYABI_ENGINEERING_BUILD
#define LOCAL_MIYABI_IPTABLE_PERMIT_PATH_3 "/system/bin/mksh"
#endif /* CONFIG_SECURITY_MIYABI_ENGINEERING_BUILD */
...
int miyabi_socket_setsockopt(struct socket *sock, int level, int optname)
{
	if(optname == IPT_SO_SET_REPLACE || optname == IPT_SO_SET_ADD_COUNTERS)
	{
		...
		if((strlen(path) == strlen(LOCAL_MIYABI_IPTABLE_PERMIT_PATH_2)) && (strcmp(path, LOCAL_MIYABI_IPTABLE_PERMIT_PATH_2) == 0))
		{
			kfree(pathbuff);
			return 0;
		}
...
#ifdef CONFIG_SECURITY_MIYABI_ENGINEERING_BUILD
		if((strlen(path) == strlen(LOCAL_MIYABI_IPTABLE_PERMIT_PATH_3)) && (strcmp(path, LOCAL_MIYABI_IPTABLE_PERMIT_PATH_3) == 0))
		{
			kfree(pathbuff);
			return 0;
		}
