...
/* [WLAN][SHARP] 2014.02.27 Correspondence that can not be the WLAN device Off Start */
	int boot_mode;
	struct wake_lock wcnss_dev_wake_lock;
/* [WLAN][SHARP] 2014.02.27 Correspondence that can not be the WLAN device Off  End  */
} *penv = NULL;
...
/* COORDINATOR Qualcomm_CPL2 BUILDERR MODIFY start */
/* Reason: Not merged board-8974-gpiomux.c */
#if 0
		...
		reg_val = 0;
#endif /* #if 0 */
/* COORDINATOR Qualcomm_CPL2 BUILDERR MODIFY end */
...
/* [WLAN][SHARP] 2014.02.27 Correspondence that can not be the WLAN device Off Start */
	if (penv->boot_mode == SH_BOOT_D || penv->boot_mode == SH_BOOT_F_F) {
		wake_unlock(&penv->wcnss_dev_wake_lock);
		wake_lock_destroy(&penv->wcnss_dev_wake_lock);
	}
/* [WLAN][SHARP] 2014.02.27 Correspondence that can not be the WLAN device Off  End  */
...
/* [WLAN][SHARP] 2014.02.27 Correspondence that can not be the WLAN device Off Start */
	...
	if (penv->boot_mode == SH_BOOT_D || penv->boot_mode == SH_BOOT_F_F) {
		wake_lock_init(&penv->wcnss_dev_wake_lock, WAKE_LOCK_SUSPEND, "wcnss_dev");
		wake_lock(&penv->wcnss_dev_wake_lock);
	}
/* [WLAN][SHARP] 2014.02.27 Correspondence that can not be the WLAN device Off  End  */
