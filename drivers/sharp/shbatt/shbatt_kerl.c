...
//	#define SHBATT_ENABLE_WAIT_AVE_V_AND_I
	...
	#undef SHBATT_ENABLE_IBAT_IADC2_BMS
	#endif
#else
...
static enum power_supply_property shbatt_ps_battery_props[] =
{
	POWER_SUPPLY_PROP_TYPE,
...
static ssize_t shbatt_drv_battery_power_supply_changed( const ptrdiff_t property, int intval );
static int shbatt_ps_property_is_writeable(struct power_supply *psy, enum power_supply_property psp);
static void shbatt_drv_external_power_changed(struct power_supply *psy);

static int shbatt_ps_get_prop_online(struct device *dev);

static int shbatt_ps_battery_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val);
static int shbatt_ps_battery_set_property(struct power_supply *psy, enum power_supply_property psp, const union power_supply_propval *val);

static int shbatt_ps_battery_get_prop_status( void );
static int shbatt_ps_battery_get_prop_health( void );
static int shbatt_ps_battery_get_prop_present( void );
static int shbatt_ps_battery_get_prop_capacity( void );
static int shbatt_ps_battery_get_prop_batt_vol( void );
static int shbatt_ps_battery_get_prop_batt_temp( void );
static int shbatt_ps_battery_get_prop_technology( void );
static int shbatt_ps_battery_get_prop_current_now( void );
static int shbatt_ps_battery_get_prop_voltage_now( void );
static int shbatt_ps_battery_get_prop_terminal_temp( void );
static int shbatt_ps_battery_get_prop_camera_temp( void );
static int shbatt_ps_battery_get_prop_safety( void );
static int shbatt_ps_battery_get_prop_voltage_max_design( void );
static int shbatt_ps_battery_get_prop_voltage_min_design( void );
static int shbatt_ps_battery_get_prop_energy_full( void );
static int shbatt_ps_battery_get_prop_charge_type( void );
static int shbatt_ps_battery_get_prop_input_current_settled( void );
static int shbatt_ps_battery_get_prop_input_current_max( void );
static int shbatt_ps_battery_get_prop_voltage_min( void );
...
static struct device_attribute shbatt_battery_attributes[] = 
{
	...
	/* power_supply_changed item never regist */
};
...
static int shbatt_before_batt_therm = -255;
static const int shbatt_before_batt_therm_coefficient = 3;
static const int shbatt_now_batt_therm_coefficient = 1;
...
static int shbatt_calc_base_update = false;
static int shbatt_calc_base_enable = false;
...
static int shbatt_drv_ioctl_cmd_notify_charge_state_machine_status( struct file* fi_p, unsigned long arg );
shbatt_result_t shbatt_api_notify_charge_state_machine_status( shchg_state_t state );
static void shbatt_task_cmd_notify_charge_state_machine_status( shbatt_packet_t* pkt_p );
static shbatt_result_t shbatt_seq_notify_charge_state_machine_status( shchg_state_t state );
...
static int shbatt_drv_ioctl_cmd_read_calc_base_capacity( struct file* fi_p, unsigned long arg );
...
static int shbatt_drv_ioctl_cmd_get_real_battery_capacity( struct file* fi_p, unsigned long arg );
...
static shbatt_result_t shbatt_seq_update_calc_base_capacity( int capacity );
static shbatt_result_t shbatt_seq_update_calc_base_bat_therm( int bat_therm );
static shbatt_result_t shbatt_seq_store_calc_base_info( void );
static shbatt_result_t shbatt_seq_read_calc_base_capacity( shbatt_calc_base_info_t* info_p );
...
static int shbatt_drv_ioctl_cmd_get_ibat_max( struct file* fi_p, unsigned long arg);
...
static void (*const shbatt_task_cmd_func[])( shbatt_packet_t* pkt_p ) =
{
	...
	shbatt_task_cmd_notify_charge_state_machine_status			/*SHBATT_TASK_CMD_NOTIFY_CHARGE_STATE_MACHINE_STATUS*/
...
shbatt_result_t shbatt_api_get_battery_capacity( int* cap_p )
{
	...
	{
		/* update calc base parameter */
		shbatt_seq_update_calc_base_capacity(*cap_p);
	}
...
shbatt_result_t shbatt_api_read_calc_base_capacity( shbatt_calc_base_info_t* info_p )
{
	shbatt_result_t result = SHBATT_RESULT_SUCCESS;

	SHBATT_TRACE("[S] %s \n",__FUNCTION__);

	result = shbatt_seq_read_calc_base_capacity(info_p);
...
int shbatt_api_get_smem_battery_temperature( void )
{
	shbatt_result_t result;
	shbatt_smem_info_t smem;
...
static shbatt_result_t shbatt_control_read_adc_channel( shbatt_adc_t* adc_p, uint32_t* microvolts )
{
	...
	if (adc_type == SHBATT_DRV_ADC_TYPE_VOLTAGE)
	{
		...
		rc = qpnp_vadc_read_sh(channel, &vadc_result);
	...
	{
#ifdef SHBATT_ENABLE_IBAT_IADC1_USR
		rc = qpnp_iadc_read_sh(channel, &iadc_result);
		if (rc)
		{
...
static void shbatt_control_read_adc_channel_conversion( shbatt_adc_t* adc_p, uint32_t microvolts )
{
	...
	case SHBATT_ADC_CHANNEL_ICHG:
		/* voltage(100mV offset) -> current: [I]=[V-100]x2 */
		adc_p->physical = (int32_t)(adc_p->physical - 100*1000) / 500;
		break;
...
static void shbatt_control_read_adc_channel_no_conversion( shbatt_adc_t* adc_p, uint32_t microvolts )
{
	...
	case SHBATT_ADC_CHANNEL_BAT_THERM:
		...
		{
			adc_p->physical = ((shbatt_before_batt_therm_coefficient * shbatt_before_batt_therm) + (shbatt_now_batt_therm_coefficient * (int)adc_p->physical)) / (shbatt_before_batt_therm_coefficient + shbatt_now_batt_therm_coefficient);
		}
		shbatt_before_batt_therm = (int)adc_p->physical;
		/* update calc base parameter */
		shbatt_seq_update_calc_base_bat_therm(shbatt_before_batt_therm);
		break;
...
/*| TODO: New API add point */
	...
	case SHBATT_DRV_IOCTL_CMD_GET_TPIN_STATUS:
		...
		break;
	case SHBATT_DRV_IOCTL_CMD_READ_CALC_BASE_CAPACITY:
		ret = shbatt_drv_ioctl_cmd_read_calc_base_capacity(fi_p,arg);
		break;
#ifdef CONFIG_PM_APQ_THERM
...
/*calibration commands.*/
	...
	case SHBATT_DRV_IOCTL_CMD_CPU_CLOCK_LIMIT_UNLOCK:
		...
		break;
	case SHBATT_DRV_IOCTL_CMD_NOTIFY_CHARGE_STATE_MACHINE_STATUS:
		ret = shbatt_drv_ioctl_cmd_notify_charge_state_machine_status(fi_p,arg);
		break;
	case SHBATT_DRV_IOCTL_CMD_GET_IBAT_MAX:
		ret = shbatt_drv_ioctl_cmd_get_ibat_max(fi_p,arg);
		break;
	case SHBATT_DRV_IOCTL_CMD_GET_REAL_BATTERY_CAPACITY:
		ret = shbatt_drv_ioctl_cmd_get_real_battery_capacity(fi_p,arg);
		break;
	default:
...
{
	int ret = 0;
	shbatt_calc_base_info_t info;

	SHBATT_TRACE("[S] %s \n",__FUNCTION__);

	if(shbatt_api_read_calc_base_capacity( &info ) != SHBATT_RESULT_SUCCESS) {
		SHBATT_ERROR("%s : shbatt_api_read_calc_base_capacity failed.\n",__FUNCTION__);
		ret = -EPERM;
		goto error;
	}

	if(copy_to_user((int __user *)arg, &info, sizeof(shbatt_calc_base_info_t)) != 0)
	{
		SHBATT_ERROR("%s : copy_to_user failed.\n",__FUNCTION__);
		ret = -EPERM;
		goto error;
...
static shbatt_result_t shbatt_seq_update_calc_base_capacity( int capacity )
{
	...
	if (shbatt_calc_base_capacity != capacity)
	{
		shbatt_calc_base_capacity = capacity;
		shbatt_calc_base_update = true;
	}

	if (shbatt_calc_base_bat_therm == SHBATT_INVALID_CALC_BASE_BAT_THERM)
	{
		SHBATT_TRACE("%s : shbatt_calc_base_bat_therm = %d\n", __FUNCTION__, shbatt_calc_base_bat_therm);
	}
	else
	{
		result = shbatt_seq_store_calc_base_info();
	}
...
static shbatt_result_t shbatt_seq_update_calc_base_bat_therm( int bat_therm )
{
	...
	if (shbatt_calc_base_bat_therm != bat_therm)
	{
		shbatt_calc_base_bat_therm = bat_therm;
		shbatt_calc_base_update = true;
	}

	if (shbatt_calc_base_capacity == SHBATT_INVALID_CALC_BASE_CAPACITY)
	{
		SHBATT_TRACE("%s : shbatt_calc_base_capacity = %d\n", __FUNCTION__, shbatt_calc_base_capacity);
	}
	else
	{
		result = shbatt_seq_store_calc_base_info();
...
static shbatt_result_t shbatt_seq_store_calc_base_info( void )
{
	...
	if (shbatt_calc_base_enable == true)
	{
		rc = qpnp_bms_store_battery_info(shbatt_calc_base_capacity, shbatt_calc_base_bat_therm);
		if (rc < 0)
		{
			SHBATT_ERROR("%s : qpnp_bms_store_battery_info failed. reg = %d\n", __FUNCTION__, rc);
			result = SHBATT_RESULT_FAIL;
		}
		else
		{
			shbatt_calc_base_update = false;
		}
	}
...
{
	shbatt_result_t result = SHBATT_RESULT_SUCCESS;
	struct qpnp_bms_calc_base_info info;
	int rc;

	SHBATT_TRACE("[S] %s \n",__FUNCTION__);

	rc = qpnp_bms_read_battery_capacity(&info);
	if (rc < 0)
	{
		SHBATT_ERROR("%s : qpnp_bms_read_battery_capacity failed. reg = %d\n", __FUNCTION__, rc);
		result = SHBATT_RESULT_FAIL;
	}
	else
	{
		info_p->capacity = info.capacity;
		info_p->bat_therm[0] = info.pre_bat_therm;
		info_p->bat_therm[1] = info.pon_bat_therm;
		info_p->rtc_data[0] = info.pre_rtc_data;
		info_p->rtc_data[1] = info.pon_rtc_data;
		info_p->pon_pbl_status = info.pon_pbl_status;
		shbatt_calc_base_enable = true;
	}
...
static int shbatt_drv_probe( struct platform_device* dev_p )
{
	...
	limit_lock_init(&shbatt_limit_lock[13], PERF_LOCK_2265600KHz, "shbatt_lowbatt_limit_13");
#endif /* SHBATT_ENABLE_LIMIT_LOCK */
...
static int shbatt_drv_create_ps_device_file( struct device* dev_p, struct device_attribute* attr_p )
{
	int ret;
...
static int shbatt_drv_create_ps_attrs( struct platform_device* dev_p )
{
	...
	for (idx = 0; idx < ARRAY_SIZE(shbatt_power_supplies); idx++)
	{
		...
		if (idx == SHBATT_PS_CATEGORY_BATTERY)
		{
			/* power supply battery */
			shbatt_power_supplies[idx].properties = shbatt_ps_battery_props;
			shbatt_power_supplies[idx].num_properties = ARRAY_SIZE(shbatt_ps_battery_props);
			shbatt_power_supplies[idx].property_is_writeable = shbatt_ps_property_is_writeable;
	...
	{
		psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_BATTERY];
		attr = &shbatt_battery_attributes[idx];
		ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
		if (ret < 0)
		{
			return ret;
		}
	...
	/* cable(ac/usb/wireless) attributes */
	psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_USB];
	attr = &shbatt_cable_status_attributes[SHBATT_PS_PROPERTY_ONLINE];
	ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
	if (ret < 0)
	{
		return ret;
	...
	psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_AC];
	attr = &shbatt_cable_status_attributes[SHBATT_PS_PROPERTY_ONLINE];
	ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
	if (ret < 0)
	{
	...
	psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_WIRELESS];
	attr = &shbatt_cable_status_attributes[SHBATT_PS_PROPERTY_ONLINE];
	ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
	if (ret < 0)
	{
	...
	{
		psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_FUELGAUGE];
		attr = &shbatt_fuelgauge_attributes[idx];
		ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
		if (ret < 0)
		{
			return ret;
	...
	/* for add capacity to fuelgauge power_supply */
	psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_FUELGAUGE];
	attr = &shbatt_battery_attributes[SHBATT_PS_PROPERTY_CAPACITY];
	ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
	if (ret < 0)
	{
		return ret;
	...
	{
		psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_PMIC];
		attr = &shbatt_pmic_attributes[idx];
		ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
		if (ret < 0)
		{
			return ret;
	...
	{
		psy = &shbatt_power_supplies[SHBATT_PS_CATEGORY_ADC];
		attr = &shbatt_adc_attributes[idx];
		ret = shbatt_drv_create_ps_device_file(psy->dev, attr);
		if (ret < 0)
		{
			return ret;
...
static int shbatt_drv_get_ps_property( struct power_supply* psy_p, enum power_supply_property psp, union power_supply_propval* val_p )
{
	...
	if (psp == POWER_SUPPLY_PROP_ONLINE)
	{
		...
		val_p->intval = shbatt_ps_get_prop_online(psy_p->dev);
		return 0;
	}

	ret = shbatt_ps_battery_get_property(psy_p, psp, val_p);
	if (ret == 0)
	{
		return 0;
	}
...
static int shbatt_ps_get_prop_online(struct device *dev)
{
	...
	if (dev == shbatt_power_supplies[SHBATT_PS_CATEGORY_USB].dev)
	{
		ret = shbatt_api_get_charger_cable_status(&cbs);
		intval = ((ret == SHBATT_RESULT_SUCCESS) && (cbs == SHBATT_CABLE_STATUS_USB)) ? 1 : 0;
		SHBATT_TRACE("/usb/online = %d\n", intval);
	}
	else
	if (dev == shbatt_power_supplies[SHBATT_PS_CATEGORY_AC].dev)
	{
		ret = shbatt_api_get_charger_cable_status(&cbs);
		intval = ((ret == SHBATT_RESULT_SUCCESS) && (cbs == SHBATT_CABLE_STATUS_AC)) ? 1 : 0;
		SHBATT_TRACE("/ac/online = %d\n", intval);
	}
	else
	if (dev == shbatt_power_supplies[SHBATT_PS_CATEGORY_WIRELESS].dev)
	{
#ifdef CONFIG_PM_WIRELESS_CHARGE
		ret = shbatt_is_wireless_charging();
		intval = ((ret == 0) && (cbs == SHBATT_CABLE_STATUS_NONE)) ? 1 : 0;
#else  /* CONFIG_PM_WIRELESS_CHARGE */
		intval = 0;
#endif /* CONFIG_PM_WIRELESS_CHARGE */
...
static int shbatt_ps_battery_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		val->intval = shbatt_ps_battery_get_prop_status();
		SHBATT_DEBUG_PRINT(debug_uevent, POWER_SUPPLY_PROP_STATUS, val->intval);
		break;

	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		val->intval = shbatt_ps_battery_get_prop_charge_type();
		SHBATT_DEBUG_PRINT(debug_uevent, POWER_SUPPLY_PROP_CHARGE_TYPE, val->intval);
		break;

	case POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED:
		val->intval = shbatt_ps_battery_get_prop_input_current_settled();
		SHBATT_DEBUG_PRINT(debug_uevent, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED, val->intval);
		break;

	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		val->intval = shbatt_ps_battery_get_prop_input_current_max();
		SHBATT_DEBUG_PRINT(debug_uevent, POWER_SUPPLY_PROP_INPUT_CURRENT_MAX, val->intval);
		break;

	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		val->intval = shbatt_ps_battery_get_prop_voltage_min();
		SHBATT_DEBUG_PRINT(debug_uevent, POWER_SUPPLY_PROP_VOLTAGE_MIN, val->intval);
		break;
...
static void shbatt_ps_battery_get_prop_status_for_debug( shbatt_chg_status_t *cgs_p )
{
	shbatt_chg_status_t cgs = *cgs_p;
	shbatt_cable_status_t cbs;

	do
	{
		if (!shchg_api_is_debug_charge())
		{
			break;
		}

		if (cgs != SHBATT_CHG_STATUS_FULL)
		{
			break;
		}
		...
		if (shbatt_api_get_charger_cable_status(&cbs) == SHBATT_RESULT_SUCCESS)
		{
			switch (cbs)
			{
			case SHBATT_CABLE_STATUS_USB:
			case SHBATT_CABLE_STATUS_AC:
			case SHBATT_CABLE_STATUS_BOTH:
				/* for debug */
				cgs = SHBATT_CHG_STATUS_CHARGING;
				break;
	...
	if (*cgs_p != cgs)
	{
		SHBATT_TRACE("/battery/status = %d -> %d\n", *cgs_p, cgs);
		*cgs_p = cgs;
	}
...
static int shbatt_ps_battery_get_prop_status( void )
{
	...
	do
	{
		...
		if (batt_auth == 0)
		{
			cgs = SHBATT_CHG_STATUS_NOT_CHARGING;
			break;
		}
		...
		if (shbatt_api_get_battery_charging_status(&cgs) == SHBATT_RESULT_SUCCESS)
		{
			SHBATT_TRACE("/battery/status = %d\n", cgs);
		}

		shbatt_ps_battery_get_prop_status_for_debug(&cgs);
	}
	while(0);
...
static int shbatt_ps_battery_get_prop_present( void )
{
	shbatt_present_t pre = SHBATT_PRESENT_BATT_NONE;
...
static int shbatt_ps_battery_get_prop_capacity( void )
{
	...
	do
	{
		if (DEBUG_CAPACITY_IS_ENABLE())
		{
			intval = debug_capacity;
			break;
		}
...
{
	shbatt_technology_t tec;
	int intval = SHBATT_PS_TECH_BATT_UNKNOWN;

	if (shbatt_api_get_battery_technology(&tec) == SHBATT_RESULT_SUCCESS)
	{
		SHBATT_TRACE("/battery/technology = %d\n", tec);
		if (tec == SHBATT_TECHNOLOGY_BATT_LIPO)
		{
			intval = SHBATT_PS_TECH_BATT_LIPO;
		}
		else
		if (tec == SHBATT_TECHNOLOGY_BATT_LION)
		{
			intval = SHBATT_PS_TECH_BATT_LION;
		}
	}
...
static int shbatt_ps_battery_get_prop_terminal_temp( void )
{
	...
	/* cannot read charge temp */
	if (shbatt_seq_get_pmic_temperature(&tmp) == SHBATT_RESULT_SUCCESS)
	{
		intval = tmp;
	}
...
static int shbatt_ps_battery_get_prop_camera_temp( void )
{
	...
	if (shbatt_seq_get_camera_temperature(&tmp) == SHBATT_RESULT_SUCCESS)
	{
		intval = tmp;
	}
...
static int shbatt_ps_battery_get_prop_safety( void )
{
	shbatt_safety_t safety = SHBATT_SAFETY_BATT_UNKNOWN;

	if (shbatt_api_get_battery_safety(&safety) == SHBATT_RESULT_SUCCESS)
	{
		SHBATT_TRACE("/battery/safety = %d\n", safety);
	}
...
static int shbatt_ps_battery_get_prop_voltage_max_design( void )
{
	int intval = SHBATT_PS_VALUE_FULL_VOLTAGE * 1000;

	SHBATT_TRACE("/battery/voltage_max_design = %d\n", intval);
...
static int shbatt_ps_battery_get_prop_voltage_min_design( void )
{
	int intval = SHBATT_PS_VALUE_EMPTYE_VOLTAGE * 1000;

	SHBATT_TRACE("/battery/voltage_min_design = %d\n", intval);
...
static int shbatt_ps_battery_get_prop_energy_full( void )
{
	int intval = SHBATT_PS_VALUE_FULL_CAPACITY * 1000 * 1000;
...
static int shbatt_ps_battery_get_prop_charge_type( void )
{
	int intval = SHBATT_CHARGE_TYPE_FAST;

	SHBATT_TRACE("/battery/charge_type = %d\n", intval);
...
static int shbatt_ps_battery_get_prop_input_current_settled( void )
{
	int intval = SHBATT_PS_VALUE_INPUT_CURRENT_SETTLED;

	SHBATT_TRACE("/battery/input_current_settled = %d\n", intval);
...
static int shbatt_ps_battery_get_prop_input_current_max( void )
{
	int intval = SHBATT_PS_VALUE_INPUT_CURRENT_MAX * 1000;

	SHBATT_TRACE("/battery/input_current_max = %d\n", intval);
...
static int shbatt_ps_battery_get_prop_voltage_min( void )
{
	int intval = SHBATT_PS_VALUE_VOLTAGE_MIN * 1000;

	SHBATT_TRACE("/battery/voltage_min = %d\n", intval);
...
static ssize_t shbatt_drv_show_cable_status_property( struct device* dev_p, struct device_attribute* attr_p ,char* buf_p )
{
	...
	case SHBATT_PS_PROPERTY_ONLINE:
		intval = shbatt_ps_get_prop_online(dev_p);
...
static ssize_t shbatt_drv_show_battery_property( struct device* dev_p, struct device_attribute* attr_p ,char* buf_p )
{
	...
	static const char *tech_text[] =
	{
		"Unknown", "Li-ion Polymer", "Li-ion"
	};
	static const char *safety_text[] =
	{
		"Unknown", "Good", "Over charging"
	};
	static const char *charge_type[] =
	{
	...
	case SHBATT_PS_PROPERTY_STATUS:
		intval = shbatt_ps_battery_get_prop_status();
		strval = status_text[intval];
		SHBATT_TRACE("/battery/status = %s\n", strval);
		size = snprintf(buf_p, PAGE_SIZE, "%s\n", strval);
		shbatt_drv_battery_power_supply_changed(property, intval);
		break;

	case SHBATT_PS_PROPERTY_HEALTH:
		intval = shbatt_ps_battery_get_prop_health();
		strval = health_text[intval];
		SHBATT_TRACE("/battery/health = %s\n", strval);
		size = snprintf(buf_p, PAGE_SIZE, "%s\n", strval);
		break;

	case SHBATT_PS_PROPERTY_PRESENT:
		intval = shbatt_ps_battery_get_prop_present();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_CAPACITY:
		intval = shbatt_ps_battery_get_prop_capacity();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_BATT_VOL:
		intval = shbatt_ps_battery_get_prop_batt_vol();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_BATT_TEMP:
		intval = shbatt_ps_battery_get_prop_batt_temp();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_TECHNOLOGY:
		intval = shbatt_ps_battery_get_prop_technology();
		strval = tech_text[intval];
		SHBATT_TRACE("/battery/technology = %s\n", strval);
		size = snprintf(buf_p, PAGE_SIZE, "%s\n", strval);
		break;

	case SHBATT_PS_PROPERTY_CURRENT_NOW:
		intval = shbatt_ps_battery_get_prop_current_now();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_VOLTAGE_NOW:
		intval = shbatt_ps_battery_get_prop_voltage_now();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_TERMINAL_TEMP:
		intval = shbatt_ps_battery_get_prop_terminal_temp();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_CAMERA_TEMP:
		intval = shbatt_ps_battery_get_prop_camera_temp();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_SAFETY:
		intval = shbatt_ps_battery_get_prop_safety();
		strval = safety_text[intval];
		SHBATT_TRACE("/battery/safety = %s\n", strval);
		size = snprintf(buf_p, PAGE_SIZE, "%s\n", strval);
		break;

	case SHBATT_PS_PROPERTY_VOLTAGE_MAX_DESIGN:
		intval = shbatt_ps_battery_get_prop_voltage_max_design();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_VOLTAGE_MIN_DESIGN:
		intval = shbatt_ps_battery_get_prop_voltage_min_design();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_ENERGY_FULL:
		intval = shbatt_ps_battery_get_prop_energy_full();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_CHARGE_TYPE:
		intval = shbatt_ps_battery_get_prop_charge_type();
		strval = charge_type[intval];
		SHBATT_TRACE("/battery/charge_type = %s\n", strval);
		size = snprintf(buf_p, PAGE_SIZE, "%s\n", strval);
		break;

	case SHBATT_PS_PROPERTY_INPUT_CURRENT_SETTLED:
		intval = shbatt_ps_battery_get_prop_input_current_settled();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_INPUT_CURRENT_MAX:
		intval = shbatt_ps_battery_get_prop_input_current_max();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;

	case SHBATT_PS_PROPERTY_VOLTAGE_MIN:
		intval = shbatt_ps_battery_get_prop_voltage_min();
		size = snprintf(buf_p, PAGE_SIZE, "%d\n", intval);
		break;
...
static ssize_t shbatt_drv_battery_power_supply_changed( const ptrdiff_t property, int intval )
{
	...
	do
	{
		ret = psy->get_property(psy, property, &psp);
		if (ret < 0)
		{
			SHBATT_ERROR("[P] %s attr=%d get_property=%d\n", __FUNCTION__, property, ret);
			break;
		}

		if (intval == psp.intval)
		{
			/* not change */
			break;
		}
		...
		psp.intval = intval;
		ret = psy->set_property(psy, property, &psp);
		if (ret < 0)
		{
			SHBATT_ERROR("[P] %s attr=%d set_property=%d\n", __FUNCTION__, property, ret);
			break;
		}
...
//	qpnp_iadc_calibrate_for_trim();
	...
	if (qpnp_iadc_get_gain_and_offset_sh(&iadc_calib) < 0)
	{
...
static int shbatt_drv_ioctl_cmd_get_ibat_max( struct file* fi_p, unsigned long arg)
{
	...
	result = qpnp_chg_charger_ibatmax_get(&chg_current);
	if(result != 0)
	{
		SHBATT_ERROR("%s : qpnp_chg_charger_ibatmax_get failed.\n", __FUNCTION__);
		goto end;
	}

	if(copy_to_user((void*)arg, &chg_current, sizeof(chg_current)) != 0)
	{
		SHBATT_ERROR("%s : copy_to_user failed.\n", __FUNCTION__);
		result = -EPERM;
	}
...
static int shbatt_drv_ioctl_cmd_get_real_battery_capacity( struct file* fi_p, unsigned long arg )
{
	int real_cap = 0;
	shbatt_result_t result;
	result = shbatt_api_get_real_battery_capacity(&real_cap);
	if(result == SHBATT_RESULT_SUCCESS)
	{
		if(copy_to_user((void*)arg, &real_cap, sizeof(real_cap)) != 0)
		{
			SHBATT_ERROR("%s : copy_to_user failed.\n", __FUNCTION__);
			result = -EPERM;
		}
...
#else  /* SHBATT_ENABLE_LIMIT_LOCK */
static inline int shbatt_drv_ioctl_cmd_cpu_clock_limit_lock( struct file* fi_p, unsigned long arg )
{ return 0; }
static inline int shbatt_drv_ioctl_cmd_cpu_clock_limit_unlock( struct file* fi_p, unsigned long arg )
{ return 0; }
#endif /* SHBATT_ENABLE_LIMIT_LOCK */
...
static int shbatt_drv_ioctl_cmd_notify_charge_state_machine_status( struct file* fi_p, unsigned long arg )
{
	...
	SHBATT_TRACE("[S] %s \n",__FUNCTION__);
	if(copy_from_user(&state,(shchg_state_t*)arg,sizeof(shchg_state_t)) != 0)
	{
		SHBATT_ERROR("%s : copy_from_user failed.\n",__FUNCTION__);
		ret = -EPERM;
	}
	...
	{
		ret = shbatt_api_notify_charge_state_machine_status(state);
	}
	SHBATT_TRACE("[E] %s \n",__FUNCTION__);
...
shbatt_result_t shbatt_api_notify_charge_state_machine_status( shchg_state_t state )
{
	...
	pkt_p->hdr.cmd = SHBATT_TASK_CMD_NOTIFY_CHARGE_STATE_MACHINE_STATUS;
	pkt_p->hdr.cb_p = NULL;
	pkt_p->hdr.cmp_p = NULL;
	pkt_p->hdr.ret_p = NULL;
	pkt_p->prm.chg_state = state;
...
static void shbatt_task_cmd_notify_charge_state_machine_status( shbatt_packet_t* pkt_p )
{
	...
	result = shbatt_seq_notify_charge_state_machine_status( pkt_p->prm.chg_state );
...
static shbatt_result_t shbatt_seq_notify_charge_state_machine_status( shchg_state_t state )
{
	...
	shbatt_usse_pkt.hdr.cmd = SHBATT_CMD_NOTIFY_CHARGE_STATE_MACHINE_STATUS;
	shbatt_usse_pkt.hdr.ret = SHBATT_RESULT_FAIL;
	shbatt_usse_pkt.prm.chg_state = state;
