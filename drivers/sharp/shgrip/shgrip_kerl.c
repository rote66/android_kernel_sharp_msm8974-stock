...
static int shgrip_sys_request_irq(struct shgrip_drv *ctrl)
{
	...
	if (ctrl->irq_request == SHGRIP_IRQ_FREE) {
		irq_set_status_flags(gpio_to_irq(SHGRIP_GPIO_GRIP_INT), IRQ_NOAUTOEN);
								...
								struct shgrip_mode_state *state)
{
	...
	state->scmsa_mode = (rbuf[3] & BIT0) ? 1 : 0;
	state->scmsaval_mode = (rbuf[3] & BIT1) ? 1 : 0;
	state->ch0_th4_hys = (rbuf[3] & BIT2) ? 1 : 0;
	state->ch2_th4_hys = (rbuf[3] & BIT3) ? 1 : 0;
	state->ch0_th5_hys = (rbuf[3] & BIT4) ? 1 : 0;
	state->ch2_th5_hys = (rbuf[3] & BIT5) ? 1 : 0;
	state->pcovrc_mode = (rbuf[3] & BIT6) ? 1 : 0;
	state->w_quick     = (rbuf[3] & BIT7) ? 1 : 0;
	...
	SHGRIP_INFO("scmsa_mode:%d, scmsaval_mode:%d, ch0_th4_hys:%d, ch2_th4_hys:%d, ch0_th5_hys:%d, ch2_th5_hys:%d, pcovrc_mode:%d, w_quick:%d, \n",
				state->scmsa_mode, state->scmsaval_mode, state->ch0_th4_hys, state->ch2_th4_hys,
				state->ch0_th5_hys, state->ch2_th5_hys, state->pcovrc_mode, state->w_quick);
...
#if 0
/* ------------------------------------------------------------------------- */
/* shgrip_command_ch1on                                                      */
/* ------------------------------------------------------------------------- */
static int shgrip_command_ch1on(struct shgrip_drv *ctrl, unsigned char ch1onset)
{
	int ret;
	unsigned char wbuf[2], rbuf[1];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_ch1off                                                     */
/* ------------------------------------------------------------------------- */
static int shgrip_command_ch1off(struct shgrip_drv *ctrl, unsigned char ch1offset)
{
	int ret;
	unsigned char wbuf[2], rbuf[1];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_gswr3                                                      */
/* ------------------------------------------------------------------------- */
static int shgrip_command_gswr3(struct shgrip_drv *ctrl, 
								struct shgrip_params_val *val)
{
	int ret;
	unsigned char wbuf[16], rbuf[1];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_gsrd3                                                      */
/* ------------------------------------------------------------------------- */
static int shgrip_command_gsrd3(struct shgrip_drv *ctrl,
								struct shgrip_params_val *val)
{
	int ret;
	int i;
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_ch1strd                                                    */
/* ------------------------------------------------------------------------- */
static int shgrip_command_ch1strd(struct shgrip_drv *ctrl, unsigned char *status)
{
	int ret;
	unsigned char wbuf[1], rbuf[2];
	...
	*status = (rbuf[1] & BIT0);
	SHGRIP_INFO("ch1 touch:%d\n", *status);
	
	return ret;
}

/* ------------------------------------------------------------------------- */
/* shgrip_command_chprd4                                                     */
/* ------------------------------------------------------------------------- */
static int shgrip_command_chprd4(struct shgrip_drv *ctrl, struct shgrip_chprd4 *val)
{
	int ret;
	unsigned char wbuf[1], rbuf[19];
	
	SHGRIP_DBG("start\n");
	
	wbuf[0] = SHGRIP_CMD_CHPRD4;
	...
	val->ncount_ch0.high_val	= rbuf[1];
	val->ncount_ch0.low_val		= rbuf[2];
	val->nref_ch0.high_val		= rbuf[3];
	val->nref_ch0.low_val		= rbuf[4];
	val->scudata_ch0.high_val	= rbuf[5];
	val->scudata_ch0.low_val	= rbuf[6];
	val->ncount_ch1.high_val	= rbuf[7];
	val->ncount_ch1.low_val		= rbuf[8];
	val->nref_ch1.high_val		= rbuf[9];
	val->nref_ch1.low_val		= rbuf[10];
	val->scudata_ch1.high_val	= rbuf[11];
	val->scudata_ch1.low_val	= rbuf[12];
	val->ncount_ch2.high_val	= rbuf[13];
	val->ncount_ch2.low_val		= rbuf[14];
	val->nref_ch2.high_val		= rbuf[15];
	val->nref_ch2.low_val		= rbuf[16];
	val->scudata_ch2.high_val	= rbuf[17];
	val->scudata_ch2.low_val	= rbuf[18];
	
	SHGRIP_INFO("CH0 ncount:0x%04X, nref:0x%04X, scudata:0x%04X\n",
		((val->ncount_ch0.high_val  << 8) | val->ncount_ch0.low_val),
		((val->nref_ch0.high_val    << 8) | val->nref_ch0.low_val),
		((val->scudata_ch0.high_val << 8) | val->scudata_ch0.low_val));

	SHGRIP_INFO("CH1 ncount:0x%04X, nref:0x%04X, scudata:0x%04X\n",
		((val->ncount_ch1.high_val  << 8) | val->ncount_ch1.low_val),
		((val->nref_ch1.high_val    << 8) | val->nref_ch1.low_val),
		((val->scudata_ch1.high_val << 8) | val->scudata_ch1.low_val));

	SHGRIP_INFO("CH2 ncount:0x%04X, nref:0x%04X, scudata:0x%04X\n",
		((val->ncount_ch2.high_val  << 8) | val->ncount_ch2.low_val),
		((val->nref_ch2.high_val    << 8) | val->nref_ch2.low_val),
		((val->scudata_ch2.high_val << 8) | val->scudata_ch2.low_val));
...
#if 0
/* ------------------------------------------------------------------------- */
/* shgrip_command_th5off                                                     */
/* ------------------------------------------------------------------------- */
static int shgrip_command_th5off(struct shgrip_drv *ctrl, struct shgrip_threshold5_reg *reg)
{
	int ret;
	...
	wbuf[0] = SHGRIP_CMD_TH5OFF;
	wbuf[1] = reg->thr5offset;
	wbuf[2] = reg->thr5cansel_off;
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_th5clr                                                     */
/* ------------------------------------------------------------------------- */
static int shgrip_command_th5clr(struct shgrip_drv *ctrl, unsigned char ch5_cnt_clr)
{
	int ret;
	unsigned char wbuf[2], rbuf[1];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_th5valrd                                                   */
/* ------------------------------------------------------------------------- */
static int shgrip_command_th5valrd(struct shgrip_drv *ctrl, struct shgrip_threshold5_reg *reg)
{
	int ret;
	unsigned char wbuf[1], rbuf[21];
	...
	reg->nhys5_ch0.high_val		= rbuf[1];
	reg->nhys5_ch0.low_val		= rbuf[2];
	reg->nhys5_ch2.high_val		= rbuf[3];
	reg->nhys5_ch2.low_val		= rbuf[4];
	reg->nthr5_ch0.high_val		= rbuf[5];
	reg->nthr5_ch0.low_val		= rbuf[6];
	reg->nthr5_ch2.high_val		= rbuf[7];
	reg->nthr5_ch2.low_val		= rbuf[8];
	reg->th5_s_cnt.high_val		= rbuf[9];
	reg->th5_s_cnt.low_val		= rbuf[10];
	reg->th5_s_cnt_ch0.high_val		= rbuf[11];
	reg->th5_s_cnt_ch0.low_val		= rbuf[12];
	reg->th5_s_cnt_ch2.high_val	= rbuf[13];
	reg->th5_s_cnt_ch2.low_val		= rbuf[14];
	reg->th5_t_cnt.high_val		= rbuf[15];
	reg->th5_t_cnt.low_val		= rbuf[16];
	reg->th5_t_cnt_ch0.high_val		= rbuf[17];
	reg->th5_t_cnt_ch0.low_val		= rbuf[18];
	reg->th5_t_cnt_ch2.high_val	= rbuf[19];
	reg->th5_t_cnt_ch2.low_val		= rbuf[20];
	
	SHGRIP_INFO("hys5_ch0_H:0x%02x, hys5_ch0_L:0x%02x, hys5_ch2_H:0x%02x, hys5_ch2_L:0x%02x\n",
				reg->nhys5_ch0.high_val, reg->nhys5_ch0.low_val, reg->nhys5_ch2.high_val, reg->nhys5_ch2.low_val);
	
	SHGRIP_INFO("thr5_ch0_H:0x%02x, thr5_ch0_L:0x%02x, thr5_ch2_H:0x%02x, thr5_ch2_L:0x%02x\n",
				reg->nthr5_ch0.high_val, reg->nthr5_ch0.low_val, reg->nthr5_ch2.high_val, reg->nthr5_ch2.low_val);
	
	SHGRIP_INFO("th5_s_cnt_H:0x%02x, th5_s_cnt_L:0x%02x\n",
				reg->th5_s_cnt.high_val, reg->th5_s_cnt.low_val);
	
	SHGRIP_INFO("th5_s_cnt_ch0_H:0x%02x, th5_s_cnt_ch0_L:0x%02x\n",
				reg->th5_s_cnt_ch0.high_val, reg->th5_s_cnt_ch0.low_val);
	
	SHGRIP_INFO("th5_s_cnt_ch2_H:0x%02x, th5_s_cnt_ch2_L:0x%02x\n",
				reg->th5_s_cnt_ch2.high_val, reg->th5_s_cnt_ch2.low_val);
	
	SHGRIP_INFO("th5_t_cnt_H:0x%02x, th5_t_cnt_L:0x%02x\n",
				reg->th5_t_cnt.high_val, reg->th5_t_cnt.low_val);
	
	SHGRIP_INFO("th5_s_cnt_ch0_H:0x%02x, th5_s_cnt_ch0_L:0x%02x\n",
				reg->th5_t_cnt_ch0.high_val, reg->th5_t_cnt_ch0.low_val);
	
	SHGRIP_INFO("th5_s_cnt_ch2_H:0x%02x, th5_s_cnt_ch2_L:0x%02x\n",
				reg->th5_t_cnt_ch2.high_val, reg->th5_t_cnt_ch2.low_val);
...
#if 0
/* ------------------------------------------------------------------------- */
/* shgrip_command_pcovrcoff                                                  */
/* ------------------------------------------------------------------------- */
static int shgrip_command_pcovrcoff(struct shgrip_drv *ctrl, unsigned char pcovrcoff)
{
	int ret;
	unsigned char wbuf[2], rbuf[1];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_pcovrcclr                                                  */
/* ------------------------------------------------------------------------- */
static int shgrip_command_pcovrcclr(struct shgrip_drv *ctrl, unsigned char pcovrcclr)
{
	int ret;
	unsigned char wbuf[2], rbuf[1];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_pcovrcvalrd                                                */
/* ------------------------------------------------------------------------- */
static int shgrip_command_pcovrcvalrd(struct shgrip_drv *ctrl, struct shgrip_pcovrc_reg *reg)
{
	int ret;
	unsigned char wbuf[1], rbuf[7];
	...
	reg->pcovrc_r_cnt_thr.high_val		= rbuf[1];
	reg->pcovrc_r_cnt_thr.low_val		= rbuf[2];
	reg->pcovrc_r_cnt.high_val		= rbuf[3];
	reg->pcovrc_r_cnt.low_val		= rbuf[4];
	reg->pcovrc_p_cnt.high_val		= rbuf[5];
	reg->pcovrc_p_cnt.low_val		= rbuf[6];
	
	SHGRIP_INFO("pcovrc_r_cnt_thr:0x%02x, pcovrc_r_cnt_thr:0x%02x\n",
				reg->pcovrc_r_cnt_thr.high_val, reg->pcovrc_r_cnt_thr.low_val);
	
	SHGRIP_INFO("pcovrc_r_cnt:0x%02x, pcovrc_r_cnt:0x%02x\n",
				reg->pcovrc_r_cnt.high_val, reg->pcovrc_r_cnt.low_val);
	
	SHGRIP_INFO("pcovrc_p_cnt:0x%02x, pcovrc_p_cnt:0x%02x\n",
				reg->pcovrc_p_cnt.high_val, reg->pcovrc_p_cnt.low_val);
...
#if 0
/* ------------------------------------------------------------------------- */
/* shgrip_command_mostrd2                                                    */
/* ------------------------------------------------------------------------- */
static int shgrip_command_mostrd2(struct shgrip_drv *ctrl, 
									struct shgrip_mode_state2 *state)
{
	int ret;
	unsigned char wbuf[1], rbuf[4];
	...
	SHGRIP_INFO("mode:0x%02x, mode2:0x%02x, mode3:0x%02x\n", state->mode, state->mode2, state->mode3);
...
#if 0
/* ------------------------------------------------------------------------- */
/* shgrip_command_uphoseird2                                                 */
/* ------------------------------------------------------------------------- */
static int shgrip_command_uphoseird2(struct shgrip_drv *ctrl)
{
	int ret, i;
	unsigned char wbuf[1], rbuf[13];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_uphoseird3                                                 */
/* ------------------------------------------------------------------------- */
static int shgrip_command_uphoseird3(struct shgrip_drv *ctrl)
{
	int ret, i;
	unsigned char wbuf[1], rbuf[13];
...
/* ------------------------------------------------------------------------- */
/* shgrip_command_uphoseird4                                                 */
/* ------------------------------------------------------------------------- */
static int shgrip_command_uphoseird4(struct shgrip_drv *ctrl)
{
	int ret, i;
	unsigned char wbuf[1], rbuf[13];
