...
/* SHMDS_ID_0701_01 add S */
#ifdef CONFIG_SHTERM
#include <sharp/shterm_k.h>
#endif /* CONFIG_SHTERM */
/* SHMDS_ID_0701_01 add E */

/* SHMDS_ID_1902_01 add S */
#include <linux/wakelock.h>
#define SHMDS_WAKELOCK_TIMEOUT              (HZ/10)
static struct wake_lock shmds_wake_lock_timeout;
/* SHMDS_ID_1902_01 add E */
/* SHMDS_ID_1101_01 add S */
#ifdef SHMDS_DETECT
#include <linux/gpio.h>
#include <asm/atomic.h>

/* SHMDS_ID_0111_01 add S */
#ifdef CONFIG_ANDROID_ENGINEERING
static int dbg_log_level_inv_mpu_ring = 0;
module_param(dbg_log_level_inv_mpu_ring,  int, 0600);
#define SHMDS_DBG_LOG_LOW(fmt, args...)  \
	if (dbg_log_level_inv_mpu_ring > 1) { \
		printk("[SHMDS][%s] " fmt, __func__, ## args); \
	}

#define SHMDS_DBG_LOG_HIGH(fmt, args...)  \
	if (dbg_log_level_inv_mpu_ring > 0) { \
		printk("[SHMDS][%s] " fmt, __func__, ## args); \
	}
#else
#define SHMDS_DBG_LOG_LOW(fmt, args...)  ((void) 0)
#define SHMDS_DBG_LOG_HIGH(fmt, args...)  ((void) 0)
#endif
/* SHMDS_ID_0111_01 add E */

int shmds_ths_value = 48;
int shmds_duration_value = 50;
static unsigned long shmds_count = 0;
/* SHMDS_ID_1102_01 mod S */
static int shmds_buf_size = 34;
int shmds_buf_size_temp = 34;
/* SHMDS_ID_1102_01 mod E */
...
static int inv_lpa_mode(struct inv_mpu_state *st, int lpa_mode)
{
	...
	int h, i, data;
	const short hz[] = {188, 98, 42, 20, 10, 5};
	const int   filter[] = {INV_FILTER_188HZ, INV_FILTER_98HZ, 
		INV_FILTER_42HZ, INV_FILTER_20HZ, 
		INV_FILTER_10HZ, INV_FILTER_5HZ};
	...
	if (INV_MPU6500 == st->chip_type) {
		...
		h = (fifo_rate >> 1);
		i = 0;
		while ((h < hz[i]) && (i < ARRAY_SIZE(filter) - 1))
			i++;
		data = filter[i];
		d |= data;
...
#define COUNT_SATURATE_TIME_MS 32
	...
	if (result)
		return result;
/* SHMDS_ID_1201_01 mod S */
//	msleep(COUNT_SATURATE_TIME_MS);
	usleep(COUNT_SATURATE_TIME_MS * 1000);
/* SHMDS_ID_1201_01 mod E */
...
#ifdef MPL_SHTERM_ENABLE
		if (st->ped.on) {
			shterm_k_set_info(SHTERM_INFO_PEDOMETER, 1 );
		}
		else {
			shterm_k_set_info(SHTERM_INFO_PEDOMETER, 0 );
		}
#endif /* MPL_SHTERM_ENABLE */
/* SHMDS_ID_0701_01 add E */
...
/* SHMDS_ID_1101_01 add S */
    ...
    shmds_check_mot_move(indio_dev, s, 0);
#endif /* SHMDS_DETECT */
/* SHMDS_ID_1101_01 add E */
...
#define DMP_INT_PED             0x08
	...
	if (d[0] & DMP_INT_SMD) {
/* SHMDS_ID_1902_01 add S */
		wake_lock_timeout(&shmds_wake_lock_timeout , SHMDS_WAKELOCK_TIMEOUT * 3);
/* SHMDS_ID_1902_01 add E */
...
/* SHMDS_ID_1101_01 add S */
	...
	struct iio_dev *indio_dev = iio_priv_to_dev(st);
	short s[THREE_AXIS];
/* SHMDS_ID_1101_02 add S */
//	u8 data[2];
	u8 data[BYTES_PER_SENSOR];
/* SHMDS_ID_1101_02 add E */
	int ind;
	
	ind = 0;
	if (st->sensor[SENSOR_ACCEL].on) {
	inv_i2c_read(st, st->reg.raw_accel, BYTES_PER_SENSOR, data);
		for (i = 0; i < 3; i++){
			s[i] = be16_to_cpup((__be16 *)(&data[ind + i * 2]));
		}
	}else
...
irqreturn_t inv_read_fifo(int irq, void *dev_id)
{
	...
	struct inv_reg_map_s *reg;
	u64 pts1;
/* SHMDS_ID_1704_02 add S */
	static s64 start_time = 0LL;
	static s64 end_time = 0LL;
	start_time = get_time_ns();
/* SHMDS_ID_1704_02 add E */
	
/* SHMDS_ID_0110_01 mod S */
//#define DMP_MIN_RUN_TIME (37 * NSEC_PER_MSEC)
#define DMP_MIN_RUN_TIME (4 * NSEC_PER_MSEC)
/* SHMDS_ID_0110_01 mod E */
/* SHMDS_ID_0111_01 add S */
	SHMDS_DBG_LOG_HIGH("start\n");
/* SHMDS_ID_0111_01 add E */
	if (st->suspend_state){
/* SHMDS_ID_0111_01 add S */
		SHMDS_DBG_LOG_HIGH("end suspend_state\n");
/* SHMDS_ID_0111_01 add E */
		return IRQ_HANDLED;
	}
/* SHMDS_ID_0112_01 del S */
//	mutex_lock(&st->suspend_resume_lock);
/* SHMDS_ID_0112_01 del E */
	mutex_lock(&indio_dev->mlock);
/* SHMDS_ID_1701_01 add S */
    shmds_qos_start();
/* SHMDS_ID_1701_01 add E */
/* SHMDS_ID_1101_01 add S */
#ifdef SHMDS_DETECT
	reg = &st->reg;
    if ( (shmds_detect_switch == DETECT_ON) && (atomic_read(&shmds_detect_mode_flg) == SHMDS_DETECT_ECONOMIZE) )
...
/* SHMDS_ID_1101_01 del S */
#ifndef SHMDS_DETECT
	reg = &st->reg;
#endif /* SHMDS_DETECT */
/* SHMDS_ID_1101_01 del E */
	if (!(st->sensor[SENSOR_ACCEL].on |
		st->sensor[SENSOR_GYRO].on |
		st->sensor[SENSOR_COMPASS].on |
		st->sensor[SENSOR_PRESSURE].on |
	...
	if (bpm) {
		...
		/* SHMDS_ID_1704_01 add S */
		if ((st->batch.on == false) && (fifo_count > bpm))
		{
/* SHMDS_ID_0111_01 add S */
			SHMDS_DBG_LOG_HIGH("usleep(1000); check\n");
/* SHMDS_ID_0111_01 add E */
			usleep(1000);
		}
		/* SHMDS_ID_1704_01 add E */
/* SHMDS_ID_1704_02 add S */
		else if(end_time != 0LL && start_time - end_time < 100000LL)
		{
			SHMDS_DBG_LOG_HIGH("usleep(1000); check2\n");
			usleep(1000);
		}
/* SHMDS_ID_1704_02 add E */
	}
...
/* SHMDS_ID_0111_01 add S */
	SHMDS_DBG_LOG_HIGH("end flush_fifo\n");
/* SHMDS_ID_0111_01 add E */
	return IRQ_HANDLED;
}
...
/* SHMDS_ID_1902_01 add S */
void shmds_wake_lock_init(void)
{
    wake_lock_init(&shmds_wake_lock_timeout, WAKE_LOCK_SUSPEND, "shmds_wake_lock_timeout");
}

void shmds_wake_lock_destroy(void)
{
    wake_lock_destroy(&shmds_wake_lock_timeout);
}
/* SHMDS_ID_1902_01 add E */
...
/* SHMDS_ID_1104_01 add S */
static void shmds_get_detect_buf_maxmin_sub(signed long *sub)
{
    int i;
    int j;
    signed short max;
...
void shmds_check_mot_move(struct iio_dev *indio_dev, short *accData, short suspend)
{
    struct inv_mpu_state *st = iio_priv(indio_dev);
    int i;
    int j;
...
/* SHMDS_ID_1104_01 del S */
//    signed short max;
//    signed short min;
/* SHMDS_ID_1104_01 del E */
    int result;
    ...
    if (shmds_detect_switch == DETECT_ON)
    {
        if ( atomic_read(&shmds_detect_mode_flg) == SHMDS_DETECT_NORMAL )
        {
            ...
            if (( !shmds_count ) && ( !shmds_full_buffer_flg ))
            {
                ...
                {
                    shmds_tmp_buf[0][i] = accData[0];
                    shmds_tmp_buf[1][i] = accData[1];
                    shmds_tmp_buf[2][i] = accData[2];
                }
            }
            else
            {
                ...
                shmds_tmp_buf[0][SHMDS_TMP_BUFSIZE - 1] = accData[0];
                shmds_tmp_buf[1][SHMDS_TMP_BUFSIZE - 1] = accData[1];
                shmds_tmp_buf[2][SHMDS_TMP_BUFSIZE - 1] = accData[2];
            }
            ...
            {
/* SHMDS_ID_1104_01 mod S */
                shmds_get_detect_buf_maxmin_sub(sub);
                if ((sub[0] <= shmds_md_thresh) && (sub[1] <= shmds_md_thresh) && (sub[2] <= shmds_md_thresh))
                {
                    shmds_detect_buf[0][shmds_count] = accData[0];
                    shmds_detect_buf[1][shmds_count] = accData[1];
                    shmds_detect_buf[2][shmds_count] = accData[2];
                    shmds_get_detect_buf_maxmin_sub(sub);
                    if ((sub[0] > shmds_md_thresh) || (sub[1] > shmds_md_thresh) || (sub[2] > shmds_md_thresh))
                    {
...
/* SHMDS_ID_1104_01 mod E */
                    ...
                    result = inv_i2c_single_write(st, REG_ACCEL_MOT_THR, shmds_ths_value);
                    if (result)
                    {
                        pr_err("%s|%d returning %d\n", __func__, __LINE__, result);
                        return ;
                	...
                	result = inv_i2c_read(st, REG_6500_ACCEL_INTEL_CTRL, 1, &readbuff);
                    if (result)
                    {
                        pr_err("%s|%d returning %d\n", __func__, __LINE__, result);
                        return ;
                    }
                    writebuff = ( (readbuff & 0x3F) | BIT_ACCEL_INTEL_ENABLE);
                    result = inv_i2c_single_write(st, REG_6500_ACCEL_INTEL_CTRL, writebuff);
                    if (result)
                    {
                        pr_err("%s|%d returning %d\n", __func__, __LINE__, result);
                        return ;
...
/* SHMDS_ID_1104_01 add E */
            ...
            else
            {
                ...
                if ((sub[0] > shmds_md_thresh) || (sub[1] > shmds_md_thresh) || (sub[2] > shmds_md_thresh) || (suspend == 1 ))
                {
                    result = inv_i2c_single_write(st, st->reg.int_enable, BIT_DATA_RDY_EN);
                    if (result)
                    {
...
static void timer_work_acc(struct inv_mpu_state *st)
{
    ...
    reg = &st->reg;
    inv_i2c_read(st, reg->raw_accel, BYTES_PER_SENSOR, data);
    inv_report_gyro_accel(indio_dev, data, get_time_ns());
...
{
    //char gpio_data = 0;
    struct inv_mpu_state *st = container_of(work, struct inv_mpu_state, work_timerhandler); 
...
/* SHMDS_ID_0111_01 add E */
    ...
    //else if (st->irq == gpio_to_irq(GPIO_GYRO_INT))
    //{
...
void shmds_start_timer(void)
{
    ...
    {
/* SHMDS_ID_1104_01 mod S */
        timer_acc.expires = jiffies + msecs_to_jiffies(300);
/* SHMDS_ID_1104_01 mod E */
        add_timer(&timer_acc);
...
/* SHMDS_ID_1001_01 add E */

/* SHMDS_ID_1101_01 add S */
int shmds_check_mot_fifo_rate(struct inv_mpu_state *st, int fifo_rate, int accel_call)
{
    int rtn_rate = fifo_rate;
    int i;
    
    if( st->sensor[SENSOR_GYRO].on || st->sensor[SENSOR_COMPASS].on || st->sensor[SENSOR_SIXQ].on
    || st->sensor[SENSOR_LPQ].on || st->batch.timeout){
        if(shmds_detect_switch == DETECT_ON){
    ...
    if(accel_call)
    {
        if( fifo_rate == 1 || fifo_rate == 2 )
        {
            if( fifo_rate == 1 )
            {
                rtn_rate = SHMDS_DETECT_RATE1;
                shmds_buf_size_temp = SHMDS_DETECT_BUFSIZE1;
            }
            else
            {
                rtn_rate = SHMDS_DETECT_RATE2;
...
int shmds_mot_init(struct inv_mpu_state *st, int irq)
{
    int i;
    int j;
    for (i = 0; i < 3; i++)
    ...
    if (irq == gpio_to_irq(GPIO_GYRO_INT))
    {
...
ssize_t shmds_count_show(char *buf)
{
    return sprintf( buf, "%ld\n", shmds_count );
}

ssize_t shmds_ths_value_show(char *buf)
{
    return sprintf( buf, "%d\n", shmds_ths_value );
}

int shmds_ths_value_store(int data)
{
...
ssize_t shmds_duration_value_show(char *buf)
{
    return sprintf( buf, "%d\n",shmds_duration_value );
}

int shmds_duration_value_store(int data)
{
    shmds_duration_value = data;
    return 0;
}

ssize_t shmds_buf_size_temp_show(char *buf)
{
    return sprintf( buf, "%d\n",shmds_buf_size_temp );
}

int shmds_buf_size_temp_store(int data)
{
    shmds_buf_size_temp = data;
    return 0;
}

ssize_t shmds_md_thresh_show(char *buf)
{
    return sprintf( buf, "%d\n",shmds_md_thresh );
}

int shmds_md_thresh_store(int data)
{
    shmds_md_thresh = data;
    return 0;
}

ssize_t shmds_mg_to_LSB_show(char *buf)
{
    return sprintf( buf, "%d\n",shmds_mg_to_LSB );
}

int shmds_mg_to_LSB_store(int data)
{
...
ssize_t shmds_md_thresh_value_show(char *buf)
{
    return sprintf( buf, "%d\n", shmds_md_thresh_value );
}

int shmds_md_thresh_value_store(int data)
{
