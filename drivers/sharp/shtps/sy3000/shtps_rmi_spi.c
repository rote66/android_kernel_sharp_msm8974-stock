...
struct shtps_rmi_spi {
	...
	u8							dev_state;
		...
		struct shtps_touch_info		hover_reject_pen_chatt_pending;
...
static int shtps_set_pen_enable(struct shtps_rmi_spi *ts, int onoff)
{
	...
	if(onoff == 0){
		/* pen disable setting */
		shtps_rmi_write(ts, ts->map.fn12.ctrl.num[23].addr, SHTPS_PEN_ENABLE_F12_CTRL23_PARAM_DISABLE);
		shtps_rmi_write(ts, SHTPS_PEN_ENABLE_F51_REG_ADDR, SHTPS_PEN_ENABLE_F51_PARAM_DISABLE);
		ts->pen_enable = 0;
	}
	else if(onoff == 1){
		/* pen enable setting */
		shtps_rmi_write(ts, ts->map.fn12.ctrl.num[23].addr, SHTPS_PEN_ENABLE_F12_CTRL23_PARAM_ENABLE);
		shtps_rmi_write(ts, SHTPS_PEN_ENABLE_F51_REG_ADDR, SHTPS_PEN_ENABLE_F51_PARAM_ENABLE);
		ts->pen_enable = 1;
	}
	else{
		return -EINVAL;
		...
		for(i = 0; i < fingerMax; i++){
			if(ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_NO_TOUCH){
				...
				if(ts->hover_reject_pen_chatt_cnt[i] > 0 &&
					ts->hover_reject_pen_chatt_cnt[i] < SHTPS_HOVER_REJECT_1_PEN_CHATT_TAP_CNT)
				{
					SHTPS_LOG_HOVER_REJECT("<%d> notify pen chatt pending event\n", i);
					shtps_report_touch_pen_on(ts, i,
										  ts->hover_reject_pen_chatt_pending.fingers[i].x,
										  ts->hover_reject_pen_chatt_pending.fingers[i].y,
										  (ts->hover_reject_pen_chatt_pending.fingers[i].wx > ts->hover_reject_pen_chatt_pending.fingers[i].wy)?
										  ts->hover_reject_pen_chatt_pending.fingers[i].wx : ts->hover_reject_pen_chatt_pending.fingers[i].wy,
										  ts->hover_reject_pen_chatt_pending.fingers[i].wx,
										  ts->hover_reject_pen_chatt_pending.fingers[i].wy,
										  ts->hover_reject_pen_chatt_pending.fingers[i].z);
					...
					ts->report_info.fingers[i].x     = ts->hover_reject_pen_chatt_pending.fingers[i].x;
					ts->report_info.fingers[i].y     = ts->hover_reject_pen_chatt_pending.fingers[i].y;
					ts->report_info.fingers[i].wx    = ts->hover_reject_pen_chatt_pending.fingers[i].wx;
					ts->report_info.fingers[i].wy    = ts->hover_reject_pen_chatt_pending.fingers[i].wy;
					ts->report_info.fingers[i].z     = ts->hover_reject_pen_chatt_pending.fingers[i].z;
				}
			...
			if(ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_PEN){
				if( (ts->hover_reject_pen_chatt_cnt[i] > 0) &&
					(ts->hover_reject_pen_chatt_cnt[i] < SHTPS_HOVER_REJECT_1_PEN_CHATT_CNT) )
				{
					ts->hover_reject_pen_chatt_cnt[i]++;
					ts->hover_reject_pen_chatt_z_total[i] += ts->fw_report_info.fingers[i].z;

					ts->hover_reject_pen_chatt_pending.fingers[i].x = info->fingers[i].x;
					ts->hover_reject_pen_chatt_pending.fingers[i].y = info->fingers[i].y;
					ts->hover_reject_pen_chatt_pending.fingers[i].wx= info->fingers[i].wx;
					ts->hover_reject_pen_chatt_pending.fingers[i].wy= info->fingers[i].wy;
					ts->hover_reject_pen_chatt_pending.fingers[i].z = info->fingers[i].z;
			...
			/* ---------- */
			if(ts->hover_reject_pen_td_move_check_state[i] != 0){
				if(info->fingers[i].state == SHTPS_TOUCH_STATE_PEN)
				{
					unsigned short diff_x;
					unsigned short diff_y;
			...
			if((ts->is_force_hover_finger & (1 << i)) == 0){
				if((ts->is_hover_finger & (1 << i)) != 0){
					...
					if(ts->hover_reject_pen_td_chatt_state[i] == 0){
						...
						if(ts->hover_reject_pen_chatt_cnt[i] >= SHTPS_HOVER_REJECT_1_PEN_CHATT_CNT){
							if( (SHTPS_HOVER_REJECT_1_PEN_CHATT_Z_THRESHOLD_MIN <= ts->hover_reject_pen_chatt_z_total[i]) &&
								(ts->hover_reject_pen_chatt_z_total[i] <= SHTPS_HOVER_REJECT_1_PEN_CHATT_Z_THRESHOLD_MAX) )
							{
								ts->is_hover_finger &= ~(1 << i);
					...
					if(info->fingers[i].state == SHTPS_TOUCH_STATE_PEN)
					{
						...
						if( shtps_hover_reject_area_check(ts->fw_report_info.fingers[i].x, ts->fw_report_info.fingers[i].y) != 0 ){
							...
							if(ts->fw_report_info.fingers[i].z <= SHTPS_HOVER_REJECT_3_1_Z_THRESHOLD){
								is_event_drop = 1;
								if(event_drop_time < SHTPS_HOVER_REJECT_3_1_IGNORE_TIME){
									event_drop_time = SHTPS_HOVER_REJECT_3_1_IGNORE_TIME;
								}
								SHTPS_LOG_HOVER_REJECT("[3-1] <%d> detect\n", i);
							}
							else if(ts->fw_report_info.fingers[i].z <= SHTPS_HOVER_REJECT_3_2_Z_THRESHOLD){
								is_event_drop = 1;
								if(event_drop_time < SHTPS_HOVER_REJECT_3_2_IGNORE_TIME){
									event_drop_time = SHTPS_HOVER_REJECT_3_2_IGNORE_TIME;
								}
								SHTPS_LOG_HOVER_REJECT("[3-2] <%d> detect\n", i);
							}
							else if(ts->fw_report_info.fingers[i].z <= SHTPS_HOVER_REJECT_3_3_Z_THRESHOLD){
								is_event_drop = 1;
								if(event_drop_time < SHTPS_HOVER_REJECT_3_3_IGNORE_TIME){
									event_drop_time = SHTPS_HOVER_REJECT_3_3_IGNORE_TIME;
								}
		...
		for(j = 1;j < shtps_get_fingermax(ts);j++){
			...
			if(ghost != 0xFF){
				if(ts->fw_report_info.fingers[finger].state != SHTPS_TOUCH_STATE_NO_TOUCH &&
					#if 0
					ts->fw_report_info.fingers[ghost].state == SHTPS_TOUCH_STATE_NO_TOUCH &&
					#endif
...
static int shtps_init_internal_variables(struct shtps_rmi_spi *ts)
{
	...
	memset(ts->drag_hist, 0, sizeof(ts->drag_hist));
		...
		perf_lock_init(&ts->perf_lock, PERF_LOCK_1497600KHz, "shtps_perf_lock");
