...
#if defined(CONFIG_MACH_LYNX_DL40) || defined(CONFIG_MACH_LYNX_DL50) || defined(CONFIG_MACH_LYNX_GP9D)
	{
		.seq_type = SENSOR_CLK,
...
int32_t imx132_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(imx132_power_flg == 1){
...
#if defined(CONFIG_MACH_LYNX_DL40) || defined(CONFIG_MACH_LYNX_DL50)
			fps = 151648352 /
				fl_lines / ll_pclk;
#else
			fps = 142736842 /
		...
		pr_err("%s power_up failed imx132_power_flg=%d", __func__, imx132_power_flg);
	}
